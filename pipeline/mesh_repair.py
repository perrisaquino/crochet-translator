...
def _fill_holes_fan(mesh: trimesh.Trimesh) -> trimesh.Trimesh:
    ...
    """
    verts = mesh.vertices.copy()  # ndarray, not list of arrays
    ...
    for start_edge in boundary_edges:
        ...
        if loop[-1] not in adj.get(loop[0], set()):
            continue  # degenerate / non-manifold boundary — skip

        centroid = np.mean(verts[loop], axis=0)
        new_idx = len(verts)
        verts = np.vstack([verts, centroid[np.newaxis]])

        for i in range(len(loop)):
            a = loop[i]
            b = loop[(i + 1) % len(loop)]
            new_faces.append([a, b, new_idx])
...
def repair_mesh(src_path: str, dst_path: str, target_faces: int = 8000, height_cm: float | None = None) -> None:
    ...
    if height_cm is not None and height_cm > 0:
        current_height = mesh.extents.max()  # largest dimension, axis-agnostic
        if current_height > 0:
